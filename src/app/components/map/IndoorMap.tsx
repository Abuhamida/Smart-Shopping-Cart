...
const IndoorMap: React.FC = () => {
  ...
  useEffect(() => {
    async function fetchGraphData() {
      setIsLoading(true);
      try {
        const response = await fetch(`/api/map/get?id=${floor_id}`);
        if (!response.ok) throw new Error("Failed to fetch data");
        const data = await response.json();
        
        if (!Array.isArray(data)) throw new Error("Expected array of floors");
        
        setFloorsData(data.map(floor => ({
          id: floor.id,
          name: floor.name,
  ...
  const currentFloor = floorsData.length > 0 ? floorsData[currentFloorIndex] : null;
  const currentGraphData = currentFloor ? currentFloor.graphData : { vertices: [], edges: [] };
    ...
    while (unvisited.size > 0) {
      ...
      currentGraphData.edges
        ...
        .forEach((edge) => {
          ...
          const fromVertex = currentGraphData.vertices.find((v) => v.id === edge.from);
          const toVertex = currentGraphData.vertices.find((v) => v.id === edge.to);
  ...
  if (!floorsData.length) return <div>No floor data available</div>;

  return (
    <div className="relative w-full h-full bg-gray-50">
      {/* Sidebar */}
      <div className={`absolute left-0 top-0 bottom-0 bg-[#1a1f2d] transition-all duration-500 ease-in-out shadow-xl ${
      ...
      <div className={`absolute transition-all duration-500 ease-in-out ${
        ...
        >
          ...
          {currentGraphData.vertices.map((vertex) => {
            ...
            return (
              <g key={vertex.id} onClick={() => handleLocationClick(vertex)} style={{ cursor: "pointer" }}>
                <circle
                  cx={vertex.cx}
                  cy={vertex.cy}
                  r={8}
