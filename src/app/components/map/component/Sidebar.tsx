...
const Sidebar: React.FC<SidebarProps> = ({
  ...
  const [expandedSection, setExpandedSection] = useState<"route" | "floor">("route");
  ...
  return (
    <div className="h-full flex flex-col text-gray-300 bg-gray-800/80">
      {/* Sidebar Header */}
        ...
        <div className="grid grid-cols-2 gap-2">
          <button
            ...
            className={`p-2 rounded-lg transition-colors flex items-center justify-center gap-1.5 ${
              expandedSection === "route" ? "bg-blue-500/20 text-blue-400" : "text-gray-400 hover:bg-blue-500/10"
            }`}
          >
          ...
          <button
            ...
            className={`p-2 rounded-lg transition-colors flex items-center justify-center gap-1.5 ${
              expandedSection === "floor" ? "bg-blue-500/20 text-blue-400" : "text-gray-400 hover:bg-blue-500/10"
            }`}
          >
            <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              ...
              {selectedStart ? (
                <div className="flex items-center gap-2 text-green-400 p-2">
                  ...
                  <span className="font-medium">
                    {currentGraphData.vertices.find(v => v.id === selectedStart)?.object_name}
                  </span>
                </div>
              ) : (
                <div className="space-y-2">
                  <div className="relative">
                    ...
                    />
                    <svg className="w-5 h-5 absolute right-2 top-2 text-gray-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  ...
                  {searchQuery && filteredLocations.length > 0 && (
                    <div className="mt-2 space-y-1 max-h-48 overflow-y-auto rounded-lg border border-gray-700 bg-gray-800/50">
                      {filteredLocations.map((location) => (
                        ...
                        >
                          <span className="text-green-400">{location.object_name}</span>
                        </div>
                      ))}
                    </div>
                  )}
            ...
            <div className="p-3 bg-gray-800/50 rounded-lg border border-gray-700">
              ...
              {selectedEnd ? (
                <div className="flex items-center gap-2 text-red-400 p-2">
                  ...
                  <span className="font-medium">
                    {currentGraphData.vertices.find(v => v.id === selectedEnd)?.object_name}
                  </span>
                </div>
              ) : (
                <div className="space-y-2">
                    ...
                    <div className="mt-2 space-y-1 max-h-48 overflow-y-auto rounded-lg border border-gray-700 bg-gray-800/50">
                      {filteredLocations.map((location) => (
                        <div
                          ...
                          onClick={() => handleLocationSelect(location.id, false)}
                        >
                          <span className="text-red-400">{location.object_name}</span>
                        </div>
                      ))}
                    </div>
                  )}
              ...
              <div className="space-y-4">
                <div className="p-3 bg-gray-800/50 rounded-lg border border-gray-700/50">
                  <div className="flex justify-between items-center mb-2">
                    ...
                    <span className="font-semibold text-blue-400">{distance} units</span>
                  </div>
                  <button
                    onClick={clearRoute}
